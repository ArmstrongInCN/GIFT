...
色标说明：图 S4-3 的涡量场与残差色标为 $\pm 25$，比图 S4-2 的 $\pm 19$ 更宽。色限是按初值分布预设的固定常数，不随所画数据伸缩：四涡旋沿用正文图所用的 $\pm 19$ / $\pm 21$，高斯分布预设为两者均 $\pm 25$（见表 S9 记号体系与 [S4_PROTOCOL.md](docs/S4_PROTOCOL.md)），原因是该分布预先指定的示例轨迹 2265 的真值场与残差最大绝对值约为 24.06 和 24.04，超过 $\pm 19$。若预设范围会裁切所选关键帧，渲染器直接拒绝出图，而不是扩大色标。每种量仍各用一个色标、六种方法共用，未按方法分别缩放，也未裁剪或过滤任何场；具体数值记录在图的 `figure_manifest.json` 中。
