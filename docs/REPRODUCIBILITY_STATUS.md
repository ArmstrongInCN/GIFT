...
| Data generation controls | 17 bounded CPU tests passed; noise/sampling equivalence and clean integration schema checked. Later full standard and FNO data runs are separate evidence below; training on a complete regenerated collection remains unverified |
| Full standard clean CPU generation with own-attempt continuation | All 270 trajectories / 37,270 frames / 152,657,920 values were generated from seeds and explicit initial conditions without reading reference fields. Separate processes paused at validation step101 and resumed the same spectral state; total actual generation time 1,539.401 s. Full integrity verification passed in 12.437 s, but field comparison is **DIFFERENT**: aggregate relative L2 0.001559317; maximum absolute difference 0.802553177 at test ID1118, t=10 (that frame's relative L2 0.032325574). The earliest saved-field mismatch is already at t=0. No full-field threshold was invented, no first arithmetic cause established, and no downstream acceptance claimed. See [source-bound full evidence](../validation/full_standard_20260913/README.md); original inputs and solver bytes remain unchanged |
| Full paired-noise derivation with own-attempt continuation | Both conditions completed all2240 blocks from the specified released clean input, with a separate-process pause/resume at1601. All **287,293,440 float32 values** match the archived noise bitwise; independent formula replay, complete RNG states, group statistics and coordinates passed. Generation9.08/4.44s; complete verification16.56s. See [full evidence](../validation/full_noise_20260913/README.md). This is not clean reintegration, whole-H5 identity, full regenerated collection assembly or model acceptance; the original packaged inputs are unchanged |
| Full FNO training/test data integration | Independent CPU jobs completed in 4,813.907 / 4,510.378 s. Integrity passed across all 2,093,137,920 training-data values and 652,083,200 test-data values; both released-field comparisons are **DIFFERENT**. Aggregate relative L2 is 0.001538922 / 0.000422226, with independently measured maximum frame-relative L2 0.024500633 / 0.007737636. Training differences are visible at t0; test files start at t=4.1 and provide no t0 comparison. See [training-data evidence](../validation/full_fno_training_20260913/README.md) and [test-data evidence](../validation/full_fno_test_20260913/README.md). No model training, full-field tolerance or causal explanation is inferred |
| New-standard noise, sampling and partial collection | Full paired-noise formula replay passed all287,293,440 values and complete final RNG/statistics. All three independent sampling outputs passed ten-array/design/RNG replay; their nine design arrays match the old cache, while targets differ by about0.28% relativeL2 with the new clean parent. Actual assembly and all three full regenerated GIFT input gates then passed on17 manifest-listed files (1.689GB), no mock/tiny/training, in25.078s. Five FNO/short/cross slots are explicitly missing; no six-experiment collection or numerical acceptance is claimed. See [noise](../validation/new_standard_noise_20260913/README.md), [sampling](../validation/new_standard_sampling_20260913/README.md) and [real assembly](../validation/new_standard_collection_20260913/README.md) |
| Regenerated baseline input profile | Default released-file SHA retained; 29 bounded metadata/plumbing and rejection tests passed. The separate full-collection audit below subsequently exercised all four real formal-configuration input functions, not their training CLIs or upstream architectures |
| Full ten-job regenerated collection | All 32 manifest files (13,155,083,047 bytes), 19 scientific sources and 20 parent receipts passed independent integrity checks, as did all three complete derived-frame mappings and three GIFT/four formal baseline input gates. Native assembly completed in 88.000 s, but its work-only wrapper failed after its own file changed; that run remains failed. Separate read-only verification took 187.218 s without recopying arrays or retraining. See [independent evidence with preserved wrapper failure](../validation/full_generated_collection_20260913/README.md). No job slot is missing; PINN regenerated runtime input is still unsupported and initialization absent. This does not establish U-Net startup, upstream-model qualification, model training or six-experiment numerical acceptance |
| Real experimental readers on generated data | Seven top-level reader calls plus the nested short reader passed in 31.218 s on complete populations. Original M2/M3 ID/time/truth/anchor rules, S1 cross-file selections and S2 holdout/dense-parent checks passed; all separately recorded float32 bit differences were zero. Six H5 files and source/manifest bindings remained unchanged. See [reader evidence](../validation/generated_readers_20260913/README.md). No experiment main, checkpoint, model, prediction, training or validation-set computation was executed; previous numerical failures remain |
...
| PINN-KC full-budget clean GPU training | An independent noise000/known run completed from untrained initialization in 3,388.355 s, with all 31,000 NAdam updates, six STRidge rounds and the original L-BFGS limits. Full source/input/budget checks and 82/82 own-journal/export tensors passed; the unchanged comparison passed **12/12 numeric values over the three clean reference rows**. Terminal audit took 1.361 s. Coefficients are not bit-identical to the archived reference; passing its tolerance is not a sparse-structure, open-mode, all-condition or full M1 acceptance claim. See [clean full-run evidence](../validation/fresh_20260910/pinn_known_noise_000/README.md). The separate noise001 two-value failure remains unchanged |
| PINN-KC full-budget 10% noise GPU training | The independent noise010/known run completed from untrained initialization in3,637.043s: full31,000 NAdam updates, six STRidge rounds and original L-BFGS limits. All12 original scalar comparisons passed; 82/82 own-journal/export tensors matched exactly. L-BFGS converged after825 iterations/856 evaluations; terminal audit1.913s. No historical-weight bit identity, open-mode or whole-M1 acceptance is claimed; noise001's two failures remain. See [full noise010 evidence](../validation/fresh_20260910/pinn_known_noise_010/README.md) |
