...
实验文档的五幅图均由数值结果重新生成，只导出 SVG。保留既定布局、配色、方法顺序、字体、轨迹及报告时刻；只有数据、必要的轴范围、色限和解释文字随结果更新。不以方法排名或接近某组预期数值作为绘图通过条件。
...
| M2 mean error | Five-method temporal-error curve, 183 × 96 mm nominal canvas | 200 trajectories; GIFT mean over three seed-specific trajectory means; one model for each baseline; markers are measured times, PCHIP is only a visual guide |
| M2 keyframes | Scalar fields and signed residuals in aligned blocks | Prespecified trajectory 1005, seed 20260820, t = 5, 6, 7, 8; no averaging of prediction fields |
| M3 mean error | Three temporal-error panels for N64, N96 and N128, 183 × 72 mm nominal canvas | Same 200 paired trajectories; GIFT band is the pointwise range across three seed means, not a confidence interval; each baseline uses one model |
| M3 fields | Reference/prediction row and aligned signed residuals, 183 × 96 mm nominal canvas | Prespecified trajectory 1130, seed 20260820, N128, t = 6; no claim that this remains a median-error example after training |
...
The M1 input must contain all 15 completed method/condition jobs (45 parameter rows), not a partial table. These commands require the numerical completion record and verify the input hashes. `figure_manifest.json` records the exported files, renderer, source identities, statistics and SVG checks. Rendering a previously completed run checks the renderer; it does not establish that a new training run reproduced its results.
